...
this._corpo.forEach(function(c,f){var g=d._corpo[f][a];b.forEach(function(a,d){var f=b[d];e[g][0][f]+=c[f]})});this._corpo.forEach(function(b,d){var f=b[a],g=JSON.stringify(b);g=JSON.parse(g);g[a]=g[c];g[c]="- -";e[f].push(g)});var f={};for(k in e)f.z=k,e[f.z].forEach(function(a){return function(c,d){b.forEach(function(b,c){e[a.z][d][b]=converter(parseFloat(e[a.z][d][b]).toFixed(2))})}}(f)),f={z:f.z};f='<table class="'+this.classe+'" id="'+this.id+'">';var k="<tbody>";var l='<thead><tr class="'+this._classeCabecalho+
'">',g=this._cabecalho;this._rodape.length===this._cabecalho.length&&b.forEach(function(a,b){var c=converter(parseFloat(d._rodape[a]).toFixed(2));d._rodape[a]=c});for(var h in e)for(var q=e[h][0][a].replace(/ /g,"_"),m=0;m<e[h].length;m++){var n="";n=0==m?'<tr id="'+q+'" style="color:red;font-weight:bold;cursor:pointer">':'<tr class="'+q+'" style="display:none">';for(var p=0;p<e[h][m].length;p++)n+="<td>"+e[h][m][p]+"</td>";n+="</tr>";k+=n}k+="<tbody>";for(h=0;h<g.length;h++)l+="<th>"+g[h]+"</th>";
if(this._rodape.length==g.length){k+='<tfoot><tr class="'+this._classeCabecalho+'">';for(h=0;h<this._rodape.length;h++)k+="<th>"+this._rodape[h]+"</th>";k+="</tr></tfoot>"}return f+(l+"</tr></thead>"+k+"</table>")};Tabela.prototype.clickTabelaDinamica=function(){$("tbody tr").each(function(){if("undefined"!==typeof $(this).attr("id")){var a=$(this).attr("id");$("#"+a+" td").bind("click",function(){$("."+a).slideToggle();$.fn.dataTable.tables({visible:!0,api:!0}).columns.adjust()})}})};
...
Vendedor.prototype.getVendedor=function(){var a="";this.isImagem?(a+='<p id="'+this.id+'">',a+='<img class="img img-circle" style="width:48px;height:48px;" title="'+this.nome+'" src="'+this.imagem+'" alt="'+this.nome+'" />'+this.nome.split(" ")[0]):a+='<p id="'+this.id+'" imagem="'+this.imagem+'" title="'+this.nome+'"><span class="glyphicon glyphicon-user"> </span> '+this.nome;return a+=' <time style="font-weight:bold" class="text-danger">'+this.tempoAtendimento+"</time></p>"};
Vendedor.prototype.iniciaAtendimento=function(){var a=new Date;this.tempoAtendimento=(10>a.getHours()?"0":"")+a.getHours()+":"+(10>a.getMinutes()?"0":"")+a.getMinutes();$("#"+this.id).find("time").html(this.tempoAtendimento);this.dataAtendimento=a.getUTCFullYear()+"-"+(a.getUTCMonth()+1)+"-"+a.getUTCDate()};Vendedor.prototype.finalizaAtendimento=function(){var a=new Date;this.tempoFinalizaAtendimento=(10>a.getHours()?"0":"")+a.getHours()+":"+(10>a.getMinutes()?"0":"")+a.getMinutes()};
Vendedor.prototype.trocaExibicao=function(){this.isImagem=this.isImagem?!1:!0;$("#"+this.id).html(this.getVendedor())};var Grafico=function(a,b,c,d){this.setDados(a);this.dadosModificados=[];this.opcoes=b;this.cores=[];this.total=0;this.refGraf="";ClasseId.call(this,"",c,d)};Grafico.prototype=new ClasseId;Grafico.prototype.constructor=Grafico;
Grafico.prototype.setDados=function(a){Array.isArray(a)&&Array.isArray(a[0])&&1<a[0].length?this.dadosModificados instanceof Array&&1<=this.dadosModificados.length?this.dadosModificados=a:this.dados=a:console.log("Dados mal formatados.")};
